...
1. Изучила начальные условия. Популяция состоит из 5000 особей. В начальный момент времени: 30 особей инфицированы; 1 здоровая особь с иммунитетом; 
(5000 - 30 - 1) особей, воприимчивых к болезни. Задала коэффициент заболеваемости, равный 0,15, и коэффициент выздоровления, равный 0,02.
...
5. Запрограммировала систему уравнений, соответствующую 1-ому случаю ($I(0) \leq I^*$): 
```
def S1(x, t):
    dx1_0 = 0
    dx1_1 = - b*x[1]
...
6. Запрограммировала систему уравнений, соответствующую 2-ому случаю ($I(0) > I^*$): 
```
def S2(x, t):
    dx2_0 = -a*x[0]
    dx2_1 = a*x[0] - b*x[1]
...
7. Запрограммировала решение систем уравнений:
```
y1 = odeint(S1, x0, t)
y2 = odeint(S2, x0, t)
```

8. Описала построение графика для 1-ого случая ($I(0) \leq I^*$):
```
...
9. Описала построение графика для 2-ого случая ($I(0) > I^*$):
```
...
10. Собрала код программы воедино и получила следующее:
```
import math
import numpy as np
...
11. Получила следующие динамики изменения числа людей из каждой группы (см. рис. -@fig:001 и -@fig:002):
